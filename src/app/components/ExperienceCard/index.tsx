  ...
  return (
    <div>
        ...
        <Link href={link} className='experienceCard flex flex-col hover:text-secondary rounded-2xl p-6 hover:ring-2 hover:ring-secondary'>
            <h3 className='font-bold flex items-center justify-between text-base'>
                <span>{title}</span>
            ...
            <span className='font-semibold italic text-xs'>
                {dateRange}
            </span>
            <div className='mt-5 text-sm'>{desc}</div>
            <ul className='mt-5 flex flex-wrap'>
                {tags.map((tag, index) => (
                    <li className="mt-2 ring-1 ring-inset ring-purple-700/10 bg-purple-200 text-purple-800 text-xs font-medium me-2 px-3 py-1.5 rounded-full" key={index}>
                        {tag.title}
                    </li> 
                ))}
            </ul>
